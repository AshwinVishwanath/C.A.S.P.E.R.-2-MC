...
// ---------------------------------------------------------------------------
// Internal helper — resolve theme from context or optional T prop
// ---------------------------------------------------------------------------
function useT(propT) {
  const ctx = useContext(ThemeCtx);
  // propT overrides context (for test/non-Provider usage)
...
// ---------------------------------------------------------------------------
export function Cap({ T: propT, children, color, size = TYPE.cap, style }) {
  const T = useT(propT);
  if (!T) return null;
  return (
    <div style={{
...
// ---------------------------------------------------------------------------
export function Pill({ T: propT, dot, color, children, glow, size = 'md', style }) {
  const T = useT(propT);
  if (!T) return null;
  const c = color || T.muted;
  const sz = size === 'sm'
    ? { fs: 10, py: 2, px: 7, dot: 5 }
    : { fs: 11, py: 4, px: 9, dot: 6 };
  return (
    <span style={{
      display: 'inline-flex',
      alignItems: 'center',
...
export function Panel({
  T: propT, title, right, children,
  padded = true, accentColor, style,
  glass, fullPad, headerExtra, scheme,
}) {
  const T = useT(propT);
  if (!T) return null;
  const sk = SCHEME_PROPS[scheme || T.scheme || 'fusion'];
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: padded ? (fullPad ? SPACE.s5 : SPACE.s4) : 0, ...blueprintBg }}>
        {children}
      </div>
    </div>
  );
...
// ---------------------------------------------------------------------------
// Btn — variants: primary / secondary / ghost / danger / dangerSolid / accent / warn
// ---------------------------------------------------------------------------
export function Btn({
  T: propT, kind = 'secondary', disabled, onClick,
  children, full, size = 'md', icon, mono = true, style,
}) {
  const T = useT(propT);
  if (!T) return null;
  ...
  return (
    <button
      ...
      style={{ ...base, ...(variants[kind] || variants.secondary), ...style }}
    >
...
// ---------------------------------------------------------------------------
export function StatTile({ T: propT, label, value, unit, color, accent, sub, large, style }) {
  const T = useT(propT);
  if (!T) return null;
  ...
  return (
    <div style={{
      ...
      <div style={{
        fontFamily: FONT.mono,
        fontSize: large ? TYPE.dataLg : TYPE.data,
        fontWeight: 700,
        ...
        {value}
        {unit && (
          <span style={{ fontSize: TYPE.body, fontWeight: 500, color: T.muted, marginLeft: 5 }}>
            {unit}
          </span>
        )}
      </div>
...
// ---------------------------------------------------------------------------
export function Sparkline({ T: propT, data, color, h = 80, label, unit, value, scheme, showGrid = true }) {
  const T = useT(propT);
  if (!T) return null;
  const sk = SCHEME_PROPS[scheme || T.scheme || 'fusion'];
  ...
  const pts = data
    ...
    .join(' ');
  // Use a stable-ish ID based on content to avoid hydration mismatches
  const id = 'sl-' + c.replace(/[^a-z0-9]/gi, '') + data.length;
    ...
    <div style={{ position: 'relative', width: '100%' }}>
      {(label || last !== undefined) && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
          ...
          <span style={{
            ...
            {unit && (
              <span style={{ color: T.muted, fontSize: TYPE.cap, fontWeight: 500, marginLeft: 4 }}>
                {unit}
              </span>
            )}
          </span>
      ...
      >
        ...
        {showGrid && [0, 1, 2, 3].map((i) => (
          <line
            key={i}
            x1="0" y1={pad + ((h - pad * 2) / 3) * i}
            x2="100" y2={pad + ((h - pad * 2) / 3) * i}
            stroke={T.gridLine} strokeWidth="0.4"
...
// ---------------------------------------------------------------------------
export function Dot({ T: propT, color, size = 8, glow = true, pulse = false }) {
  const T = useT(propT);
  if (!T) return null;
  const c = color || T.muted;
  return (
...
// ---------------------------------------------------------------------------
// BigNum — display readout
// ---------------------------------------------------------------------------
export function BigNum({ T: propT, value, unit, size = TYPE.dataLg, color, mono = true, weight = 700, glow }) {
  const T = useT(propT);
  if (!T) return null;
  const c = color || T.strong;
  return (
...
// Toggle — pill switch
// ---------------------------------------------------------------------------
export function Toggle({ T: propT, on, onChange, label }) {
  const T = useT(propT);
  if (!T) return null;
  return (
    <label style={{
      ...
      {label && (
        <span style={{ fontFamily: FONT.mono, fontSize: TYPE.body, color: T.text }}>
          {label}
        </span>
      )}
    </label>
...
// ---------------------------------------------------------------------------
// SegToggle — horizontal segmented control
// options: [{ id, label, icon? }]
// ---------------------------------------------------------------------------
export function SegToggle({ T: propT, value, onChange, options, size = 'md' }) {
  const T = useT(propT);
  if (!T) return null;
    ...
    <div style={{
      ...
      {options.map((opt) => {
        ...
        return (
          ...
          >
            {opt.icon && <Icon name={opt.icon} size={sz.fs} />}
            {opt.label}
          </button>
        );
      })}
